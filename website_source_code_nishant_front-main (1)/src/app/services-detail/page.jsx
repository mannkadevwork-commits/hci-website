  ...
  return (
    <MainLayout>
      ...
      <style dangerouslySetInnerHTML={{__html: `
        ...
        .rich-text-content ol { display: flex; flex-direction: column; gap: 1rem; padding-left: 1.5rem; margin: 2rem 0; }
.rich-text-content ol li { background: #fdfdfd; border: 1px solid #f1f5f9; padding: 1.2rem 1.5rem; border-radius: 12px; display: list-item; position: relative; font-weight: 500; font-family: var(--font-poppins), sans-serif; transition: 0.3s; list-style-type: decimal; }
.rich-text-content ol li:hover { border-color: var(--hc-primary); transform: translateY(-3px); box-shadow: 0 10px 20px rgba(255,145,77,0.1); }
      ...
      <main className="bg-light pb-5" style={{ overflowX: 'hidden' }}>
                      ...
                      <div className="row g-4 font-poppins mobile-slider-wrapper">
                        {recentBlogs.map((blog, idx) => (
                          <div className="col-md-6" key={idx}>
                            <Link href={`/${blog.seo_content?.slug || `blog-detail?id=${blog.id}`}`} className="text-decoration-none">
                              <div className="d-flex align-items-center border p-3 rounded-4 bg-white shadow-sm h-100 transition-all hover:shadow-md">
                                <img src={blog.image || "/images/default.jpg"} alt={blog.title} className="rounded" style={{ width: "80px", height: "80px", objectFit: "fill", aspectRatio: "1/1", flexShrink: 0 }} loading="lazy" />
                                <div className="ms-3">
                                  <h6 className="text-dark fw-bold mb-1" style={{ fontSize: '14px' }}>{blog.title.length > 45 ? `${blog.title.substring(0, 45)}...` : blog.title}</h6>
                                  <small className="text-gradient fw-bold">READ ARTICLE <FaArrowRight size={10} /></small>
                                </div>
